...
directory <- "path\to\raw\genes\counts"
...
for(file in all_files){
  ...
  file_clust <- paste0("path\to\CIMPcluster\membership",cancer,"_cluster_memb_new.csv")
  ...
  write.csv(as.data.frame(res[which(res$pvalue<0.05),c("log2FoldChange","pvalue")]),file=paste0("path\to\save\",cancer,"_diff_gene_expression.csv"))
